...
# Esta parte muestra el diagrama de arquitectura del sistema. El diagrama ASCII
# ilustra el flujo completo: los PDFs de entrada pasan por extracción de texto
# (pdfplumber), luego se dividen en chunks con tiktoken, se convierten a vectores
# con text-embedding-3-small, y se almacenan en ChromaDB. Cuando el usuario hace
# una pregunta, también se convierte a vector, se buscan los 5 chunks más
# similares, se construye un prompt con una de 4 estrategias, y GPT-4o genera
...
# ═══════════════════════════════════════════════════════════════════════════════
# 🎤 PRESENTACIÓN — Componentes del Sistema
# Esta tabla describe brevemente cada componente del sistema: config.py gestiona
# la configuración central, prompts.py define las 4 estrategias de prompt
# engineering, ingest.py hace el pipeline PDF→vectores, rag.py es el motor de
# consulta RAG, y app.py es la interfaz web en Streamlit.
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
# 🎤 PRESENTACIÓN — Impacto de la Fragmentación
# IMPACTO DE LA FRAGMENTACIÓN: El sistema indexa cada paper en DOS granularidades:
# chunks de 256 tokens (más precisos, más granulares, mejor para preguntas
# específicas) y chunks de 1024 tokens (más contexto por chunk, mejor para
# preguntas conceptuales amplias). El demo en la línea ~712 muestra cuántos
# chunks produce cada tamaño para el mismo paper.
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
# 🎤 PRESENTACIÓN — Visualización de Citas
# VISUALIZACIÓN DE CITAS: Esta función genera citas en formato APA
# automáticamente a partir de los metadatos de cada chunk recuperado. En la
# interfaz, las citas aparecen en un expander 'Ver fuentes' debajo de cada
# respuesta.
# ═══════════════════════════════════════════════════════════════════════════════
...
def ask(question: str, chunk_size: int = 256, strategy: int = 1,
    ...
    # ═══════════════════════════════════════════════════════════════════════════
    # 🎤 PRESENTACIÓN — Pregunta "No Sé" (Fallback)
    # PREGUNTA 'NO SÉ': Si el sistema no encuentra chunks relevantes (por ejemplo,
    # preguntando sobre un tema fuera del dominio como 'What is the best pizza
    # recipe?'), retorna este mensaje de fallback en lugar de alucinar. También
    # se puede demostrar preguntando algo muy específico no cubierto por los papers.
    # ═══════════════════════════════════════════════════════════════════════════
    if not chunks:
        return {
...
# ═══════════════════════════════════════════════════════════════════════════════
# 🎤 PRESENTACIÓN — Comparación de Estrategias de Prompts
# COMPARACIÓN DE ESTRATEGIAS: Esta sección ejecuta la misma pregunta con las 4
# estrategias y muestra los resultados. Permite comparar: Estrategia 1 (respuesta
# concisa con tags XML), Estrategia 2 (JSON estructurado con campo de confianza),
# Estrategia 3 (sigue el estilo de 2 ejemplos), Estrategia 4 (razonamiento
# explícito paso a paso).
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
# 🎤 PRESENTACIÓN — Pregunta Factual Simple
# PREGUNTA FACTUAL SIMPLE: '¿Cuáles son los principales métodos de ML para
# predicción de precios de acciones?' Esta es una pregunta directa con respuesta
# clara en los papers. Se usa Estrategia 1 (Delimitadores) con chunks de 256
# tokens.
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
# 🎤 PRESENTACIÓN — Métricas de Evaluación
# MÉTRICAS DE EVALUACIÓN: Cada chunk recuperado incluye su 'distance' (distancia
# coseno). Un valor cercano a 0 = muy similar a la pregunta, cercano a 2 = sin
# relación. También se puede ver el campo 'confidence' en la Estrategia 2 (JSON),
# que el modelo estima entre 0 y 1 para indicar qué tan bien respaldada está su
# respuesta.
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
# 🎤 PRESENTACIÓN — Pregunta Compleja (Múltiples Fuentes)
# PREGUNTA COMPLEJA (múltiples fuentes): '¿Cómo se usa la IA para predecir
# crisis financieras?' Requiere sintetizar información de varios papers. Se usa
# Estrategia 4 (Chain-of-Thought) con chunks de 1024 tokens para obtener más
# contexto. El modelo razona paso a paso comparando lo que dice cada fuente.
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
# 🎤 PRESENTACIÓN — Interfaz Web (Streamlit)
# Esta sección muestra la interfaz web construida con Streamlit. Tiene sidebar
# con controles (chunk size, estrategia) y 3 pestañas: Chat (conversación con
# el sistema), Papers (navegador con filtros), y Dashboard (métricas y
# visualizaciones).
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
# 🎤 PRESENTACIÓN — Navegador de Papers y Filtros
# NAVEGADOR DE PAPERS Y FILTROS: La pestaña 'Papers' muestra el catálogo
# completo de los 21 papers con un buscador de texto libre y un filtro de año
# con slider. Al seleccionar un paper, muestra el preview del primer chunk
# indexado.
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
# 🎤 PRESENTACIÓN — Visualizaciones (Dashboard)
# VISUALIZACIONES: La pestaña 'Dashboard' muestra métricas KPI (total de papers,
# chunks indexados), gráfico de barras con distribución de papers por año, y
# tabla con el uso de estrategias durante la sesión actual.
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
# 🎤 PRESENTACIÓN — Tecnologías Clave (Stack Tecnológico)
# Esta tabla resume las tecnologías clave: pdfplumber para leer PDFs, tiktoken
# para tokenizar, text-embedding-3-small para vectores semánticos (1536
# dimensiones), ChromaDB como base vectorial local, GPT-4o como LLM generador,
# y Streamlit para la interfaz web.
# ═══════════════════════════════════════════════════════════════════════════════
